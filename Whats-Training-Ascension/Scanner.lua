...
WT_AscensionAccountDB = WT_AscensionAccountDB or {}

local function dprint(...)
  if not wt.debug then return end
  DEFAULT_CHAT_FRAME:AddMessage("|cff66ccff[WT:Ascension]|r " .. table.concat({tostringall(...)}, " "))
end

local tip = CreateFrame("GameTooltip", "WT_AscensionTrainerTip", UIParent, "GameTooltipTemplate")
...
local function ReadDescription(index)
  ...
  if tip.SetTrainerService then
    ...
    for i = 3, tip:NumLines() do
      local fs = _G["WT_AscensionTrainerTipTextLeft" .. i]
...
-- Debounced timer
local throttle = CreateFrame("Frame", "WT_AscensionTrainerScanOnceThrottle")
throttle:Hide()
...
SlashCmdList["WTE"] = function(msg)
  ...
  if msg == "debug" then
    ...
    print("|cff66ccff[WT:Ascension]|r debug =", wt.debug and "ON" or "OFF")
  ...
  elseif msg == "scan" then
    ...
    if not ok then
      print("|cff66ccff[WT:Ascension]|r scan error:", tostring(err))
    end
  ...
  elseif msg == "reset" then
    if type(wt.DB_Reset) == "function" then
      ...
      WT_AscensionCharDB = { version = 1, spells = {} }
    end
    wt._iconCache = nil
    wt._trainerScannedThisSession = false
    ...
    print("|cff66ccff[WT:Ascension]|r cache cleared. Visit a class trainer to repopulate.")

  elseif msg == "test" then
    if type(wt.TestAnnounce) == "function" then
      wt.TestAnnounce()
    else
      print("|cff66ccff[WT:Ascension]|r This command only works if you have cached spells available at your class trainer.")
    end

  elseif msg == "icon" then
    -- Account-wide toggle for tab icon
    WT_AscensionAccountDB = WT_AscensionAccountDB or {}
    WT_AscensionAccountDB.useClassTabIcon = not WT_AscensionAccountDB.useClassTabIcon
    print("|cff66ccff[WT:Ascension]|r Tab icon (account-wide):", WT_AscensionAccountDB.useClassTabIcon and "Class icon" or "Question mark")
    ...
    print("|cff66ccff[WT:Ascension]|r commands: debug | scan | reset | test | icon")
  end
end
